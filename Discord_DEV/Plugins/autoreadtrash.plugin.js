 ...
 * @website https://thomasthanos.github.io/1st-theme/Discord_DEV/Plugins/autoreadtrash.plugin.js
 */
...
module.exports = class AutoReadTrash {
    ...
    checkForUpdate() {
        ...
        const currentVersion = "5.4.8";
        fetch(updateUrl)
            .then(res => res.text())
            .then(code => {
                const remoteVersion = code.match(/@version\s+([\d.]+)/)?.[1].trim();
                if (remoteVersion && remoteVersion !== currentVersion) {
                    this.isNewerVersion(remoteVersion, currentVersion) && this.promptUpdate(updateUrl, remoteVersion);
                }
            })
    ...
    promptUpdate(url, newVersion) {
        BdApi.showConfirmationModal("Ενημέρωση Plugin", `Βρέθηκε νέα έκδοση ($v${newVersion}) του AutoReadTrash!`, {
            confirmText: "Ενημέρωση τώρα",
            cancelText: "Άκυρο",
            onConfirm: () => {
